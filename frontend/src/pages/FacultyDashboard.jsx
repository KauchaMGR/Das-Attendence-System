          ...
          <Card title="Capture attendance" sub="Upload the classroom photo to run detection + recognition">
            ...
            {session && (
              <div className="flex justify-between mt-4 font-mono text-[11.5px] text-muted">
                <span>Detected: <b className="text-ink">{session.detected} / {session.enrolled}</b></span>
                <span>Avg. confidence: <b className="text-ink">{session.avgConfidence}%</b></span>
              </div>
            )}
          </Card>
          ...
          <Card title="Session summary" sub="Auto-updating as scans come in">
            <table className="w-full text-[13px]">
              <tbody>
                ...
                <tr className="border-b border-rule/70"><td className="py-2 font-mono text-muted">Absent</td><td className="py-2 text-right">{roster.filter((r) => r.status === "absent").length}</td></tr>
